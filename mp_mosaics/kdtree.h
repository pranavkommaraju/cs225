...
class KDTree
{
  ...
  private:
     ...
     */
    int partition(vector<Point<Dim>>& points, int left, int right, int pivotidx, int dim);


    int quickselect(vector<Point<Dim>>& points, int left, int right, int k, int dim);

    void buildtree(vector<Point<Dim>>& newPoints, int left, int right, KDTreeNode* curr, int dim);

    typename KDTree<Dim>::KDTreeNode* findNearest(const Point<Dim>& query, int dim, KDTreeNode* curr) const;
